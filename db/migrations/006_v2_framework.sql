-- 006_v2_framework.sql
-- v2 schema framework per /docs/SCHEMA_V2.md (sections 3-6).
-- Schema effect: v6.1 → v7.0. Additive — v1 tables (initiatives, entities, links from
-- migration 004) remain in place and untouched; v2 tables sit alongside.
--
-- Dependency order:
--   1. companies                        (no FKs)
--   2. initiatives_v2                   (FK -> companies)
--   3. attribute_definitions            (no FKs)
--   4. tech_functions                   (no FKs)
--   5. components                       (FK -> initiatives_v2; self-FK)
--   6. component_attributes             (FK -> components, attribute_definitions)
--   7. claims_v2                        (FK -> initiatives_v2, components, attribute_definitions)
--   8. schema_migrations                (no FKs)
--   then trigger function + trigger     (depends on components, attribute_definitions)
--   then views                          (depends on all above)
--   then seed: 61 attribute_definitions (13 tech / 12 each x 4 vectors)
--   then seed: schema_migrations 1-6
--
-- No BEGIN/COMMIT here — runner manages the transaction.
-- Strict spec transcription. No IF NOT EXISTS / OR REPLACE on tables — re-apply
-- protection comes from the schema_migrations ledger (PK on version).
...
-- ============================================================================
-- Trigger — auto-create component_attributes on components insert
-- ============================================================================
...
-- Views
-- ============================================================================
CREATE OR REPLACE VIEW components_incomplete AS
SELECT
...
CREATE OR REPLACE VIEW components_with_full_record AS
SELECT
  c.id AS component_id,
  c.name AS component_name,
  c.component_type,
...
-- ============================================================================
-- Seed data — attribute_definitions (61 rows)
-- ============================================================================
...
-- ============================================================================
-- Seed: schema_migrations ledger (versions 1-6, backfill + this migration)
-- ============================================================================
